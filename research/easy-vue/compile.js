...
Compile.prototype = {
    ...
    node2Fragment: function (el) {
        ...
        //child等于他的第一个节点,如果存在就加入到虚拟节点里面,直到全部加入,结束
    ...
    compileElement: function (el) {
        ...
        //[].sliec.call(),兼容的写法,把括号里面的转化成真正的数组,比如一些伪数组
...
var compileUtil = {
    ...
    bind: function (node, vm, exp, dir) {
        ...
        //实现更新显示,数据改变会动态更新(监视器)
        new Watcher(vm, exp, function (value, oldValue) { //回调函数关注的两个点:1.什么时候调用 2.用来做什么 3.this指向
            updaterFn && updaterFn(node, value, oldValue);//当表达式对应的一个属性值变化时回调
        });
    },
    ...
    eventHandler: function (node, vm, exp, dir) {
        ...
        //如果都存在
        if (eventType && fn) {
            //bind的意义是给节点绑定指定事件名和回调函数(强制绑定this为vm)的DOM事件监听
